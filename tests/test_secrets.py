...
def test_demo_mode_never_probes_keyring(monkeypatch):
    monkeypatch.setenv("YAGAMI_DEMO_MODE", "true")
    monkeypatch.setenv("YAGAMI_HEADLESS", "false")
    monkeypatch.setenv("OPENAI_API_KEY", "optional-demo-environment-key")
    monkeypatch.setattr(
        "keyring.get_password",
        lambda *_args: (_ for _ in ()).throw(AssertionError("keyring should not be called")),
    )
    ...
    assert secrets.get("OPENAI_API_KEY") == "optional-demo-environment-key"
