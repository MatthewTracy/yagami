...
class RoutingPolicy:
    ...
    async def decide(
        ...
        *,
        ...
        if override.forced_backend:
            return self._apply_override(override, last_text)

        # 2. Programmatic force_backend (WS field).
        if force_backend:
            return self._apply_force_backend(force_backend, last_text)

        # 3. Rule-based fast-path bypass for high-confidence cases.
        bypass = can_bypass(last_text)
        if bypass is not None:
            return self._apply_rules(bypass, "rules-fast-path", last_text)
        ...
        return self._apply_rules(classification, source, last_text)

    def _apply_override(self, override: OverrideResult, original_text: str) -> RoutingDecision:
        ...
        if backend is None:
            raise OverrideRefused(
                f"override backend {override.forced_backend!r} not available"
            )
        ...
        sysprompt = (
            ...
            if sensitive == Sensitivity.PHI_MEDICAL
            else None
        )
    ...
    def _apply_force_backend(self, name: str, last_text: str) -> RoutingDecision:
        from .fast_path import _has_phi, _has_secret  # local import to avoid cycle
    ...
    def _apply_rules(
        ...
        if source == "rules-fast-path":
            why += f"; short non-PHI non-image non-code"
        why += "]"
        return RoutingDecision(
            backend=default,
            reason=why,
